...
//============================== Road Kill ======================================
		if ((G_ONMISSION == 0) && (G_ASSASSIN == 1))
		{
			...
			//=============== звук звонка =============== 
			if ((IS_CHAR_IN_AREA_3D( GetPlayerPed(), 470.635+20, -490.757+20, 5.01-5, 470.635-20, -490.757-20, 5.01+10, 0 )) && (sound_load == 0))// проверка игрок в зоне загрузки звука телефонамата и не на миссии и поступил звонок "подойдите к телефону в Маленькой Гаване".
			{
			...
			if (( PlayR < 1.5) && (!IS_CHAR_SITTING_IN_ANY_CAR(GetPlayerPed())))
			{
				...
				uint CarM1 = MODEL_HELLFURY;//Переменная "Car2" =
				...
				SET_POLICE_IGNORE_PLAYER(GetPlayerIndex(), 0);// не игнорируют копы
...
//============================== Waste the Wife ======================================
		else if ((G_ONMISSION == 0) && (G_ASSASSIN == 3))
		{
			...
			//=============== звук звонка =============== 
			if ((IS_CHAR_IN_AREA_3D( GetPlayerPed(), 915.516+20, 777.734+20, 5.019-5, 915.516-20, 777.734-20, 5.019+10, 0 )) && (sound_load == 0))// проверка игрок в зоне загрузки звука телефонамата и не на миссии и поступил звонок "подойдите к телефону в Маленькой Гаване".
			{
				while (!(REQUEST_AMBIENT_AUDIO_BANK( "SCRIPT_MISSION/PHONERING" )))
				{
					 WAIT(0);
			...
			if (( PlayR < 1.5) && (!IS_CHAR_SITTING_IN_ANY_CAR(GetPlayerPed())))
			{
				...
				uint CarM1 = MODEL_COMET;//Переменная "Car2" =
				uint trubka = cj_handset_one;
				...
				while(true)
				{
					...
					if (PlayR < 3.3)
					{
						if (IS_CHAR_SITTING_IN_ANY_CAR(GetPlayerPed()))
						{
							...
							SetTime(1500);
							TASK_GO_STRAIGHT_TO_COORD(ped1, 825.357, 735.97, 5.004, 2, -2);// педе
...
//============================== Autocide ======================================
		else if ((G_ONMISSION == 0) && (G_ASSASSIN == 5))
		{
			...
			if (( PlayR < 1.5) && (!IS_CHAR_SITTING_IN_ANY_CAR(GetPlayerPed())))
			{
				...
				uint CarM1 = MODEL_PCJ;//Переменная "Car2" =
				uint CarM2 = MODEL_STOCKADE;//Переменная "Car2" =
				uint CarM3 = MODEL_BOBCAT;//Переменная "Car2" =
				uint CarM4 = MODEL_TROPIC;//Переменная "Car2" =
				...
				CREATE_CAR(CarM1, 390.58, -506.241, 4.95, &car1, TRUE);// PCJ (Белый) для
				CREATE_CAR(CarM2, 286.232, -791.79, 6.01, &car2, TRUE);// охраны
				...
				while(true)
				{
					...
					if (stage == 2)
					{
						...
						if (IS_CHAR_DEAD(ped2))// проверка игрок убил цель
						{
							...
							SET_CAR_COORDINATES(car3, 839.6844, 743.52, 4.79);
...
//============================== Check Out at the Check In ======================================
		else if ((G_ONMISSION == 0) && (G_ASSASSIN == 7))
		{
			...
			//=============== звук звонка =============== 
			if ((IS_CHAR_IN_AREA_3D( GetPlayerPed(), -1050.36+20, -291.03+20, 8.83-5, -1050.36-20, -291.03-20, 8.83+10, 0 )) && (sound_load == 0))// проверка игрок в зоне загрузки звука телефонамата и не на миссии и поступил звонок "подойдите к телефону в Маленькой Гаване".
			{
				while (!(REQUEST_AMBIENT_AUDIO_BANK( "SCRIPT_MISSION/PHONERING" )))
				{
					 WAIT(0);
			...
			if (( PlayR < 1.5) && (!IS_CHAR_SITTING_IN_ANY_CAR(GetPlayerPed())))
			{
				...
				uint CarM1 = MODEL_ADMIRAL;//Переменная "Car2" =
				uint CarM2 = MODEL_WASHINGTON;//Переменная "Car2" =
				...
				CREATE_CHAR (26, PedM1, 290.008, -783.209, 5.06, &ped1, TRUE);
				CREATE_CHAR (26, PedM2, 286.386, -785.672, 5.06, &ped2, TRUE);
				CREATE_CHAR (26, PedM3, 289.619, -785.672, 5.06, &ped3, TRUE);
				CREATE_CHAR (26, PedM4, 292.961, -785.672, 5.06, &ped14, TRUE);
				CREATE_CHAR (26, PedM3, 296.473, -785.672, 5.06, &ped15, TRUE);
				CREATE_CHAR (26, PedM3, 299.976, -785.672, 5.06, &ped16, TRUE);
				CREATE_CHAR (26, PedM4, 286.386, -783.209, 5.06, &ped17, TRUE);
...
//============================== Loose Ends ======================================
		else if ((G_ONMISSION == 0) && (G_ASSASSIN == 9))
		{
			...
			//=============== звук звонка =============== 
			if ((IS_CHAR_IN_AREA_3D( GetPlayerPed(), -544.91+20, 2.23+20, 4.81-5, -544.91-20, 2.23-20, 4.81+10, 0 )) && (sound_load == 0))// проверка игрок в зоне загрузки звука телефонамата и не на миссии и поступил звонок "подойдите к телефону в Маленькой Гаване".
			{
				while (!(REQUEST_AMBIENT_AUDIO_BANK( "SCRIPT_MISSION/PHONERING" )))
				{
					 WAIT(0);
			...
			if (( PlayR < 1.5) && (!IS_CHAR_SITTING_IN_ANY_CAR(GetPlayerPed())))
			{
				...
				uint CarM1 = MODEL_ADMIRAL;//Переменная "Car2" =
				uint CarM2 = MODEL_PONY;//Переменная "Car2" =
				uint CarM3 = MODEL_MAVERICK;//Переменная "Car2" =
				uint barrel = barrel4;
				...
				REQUEST_MODEL(CarM2);
				while (!HAS_MODEL_LOADED(CarM2));
				CREATE_CAR(CarM2, -492.686, 5.85, 6.267, &car2, TRUE);
				SET_CAR_HEADING(car2, 95.0);

				REQUEST_MODEL(CarM3);
				while (!HAS_MODEL_LOADED(CarM3));
				CREATE_CAR(CarM3, -439.053, 18.085, 23.481, &car3, TRUE);
				...
				REQUEST_MODEL(PedM1);
				while (!HAS_MODEL_LOADED(PedM1));
				CREATE_CHAR (26, PedM1, -504.208, 22.287, 5.197, &ped1, TRUE);
				SET_CHAR_HEADING(ped1, 98.5);

				REQUEST_MODEL(PedM2);
				while (!HAS_MODEL_LOADED(PedM2));
				CREATE_CHAR (26, PedM2, -503.577, 18.338, 5.161, &ped2, TRUE);
				SET_CHAR_HEADING(ped2, 98.5);

				REQUEST_MODEL(PedM3);
				while (!HAS_MODEL_LOADED(PedM3));
				CREATE_CHAR (26, PedM3, -455.159, 14.935, 23.907, &ped3, TRUE);
				SET_CHAR_HEADING(ped3, 68.5);

				REQUEST_MODEL(PedM4);
				while (!HAS_MODEL_LOADED(PedM4));
				CREATE_CHAR (26, PedM4, -461.066, 14.043, 23.907, &ped4, TRUE);
				SET_CHAR_HEADING(ped4, -106.5);

				CREATE_CHAR (26, PedM1, -492.001, -1.29, 6.108, &ped5, TRUE);
				CREATE_CHAR (26, PedM2, -492.665, 4.217, 6.186, &ped6, TRUE);
				CREATE_CHAR (26, PedM3, -476.242, -17.131, 6.267, &ped7, TRUE);
				CREATE_CHAR (26, PedM1, -487.712, -11.489, 6.262, &ped8, TRUE);
				CREATE_CHAR (26, PedM2, -493.399, -23.607, 5.496, &ped9, TRUE);
				CREATE_CHAR (26, PedM3, -495.069, -43.556, 5.243, &ped10, TRUE);
				CREATE_CHAR (26, PedM1, -491.706, -51.346, 5.481, &ped11, TRUE);
				CREATE_CHAR (26, PedM2, -502.987, -58.548, 4.928, &ped12, TRUE);
				CREATE_CHAR (26, PedM3, -499.182, -71.402, 6.993, &ped13, TRUE);
				CREATE_CHAR (26, PedM1, -494.798, -71.694, 6.981, &ped14, TRUE);
				CREATE_CHAR (26, PedM2, -489.305, -94.309, 10.624, &ped15, TRUE);
				CREATE_CHAR (26, PedM3, -476.489, -91.927, 11.093, &ped16, TRUE);
				CREATE_CHAR (26, PedM1, -444.421, -95.484, 10.468, &ped17, TRUE);
				CREATE_CHAR (26, PedM2, -441.473, -72.128, 10.377, &ped18, TRUE);
				CREATE_CHAR (26, PedM3, -449.371, -58.777, 14.305, &ped19, TRUE);
				CREATE_CHAR (26, PedM1, -445.028, -37.545, 14.305, &ped20, TRUE);
				CREATE_CHAR (26, PedM2, -451.341, -15.893, 23.867, &ped21, TRUE);
				CREATE_CHAR (26, PedM3, -446.177, -10.209, 23.867, &ped22, TRUE);
				CREATE_CHAR (26, PedM1, -458.958, -19.921, 19.459, &ped23, TRUE);
				CREATE_CHAR (26, PedM2, -465.429, -16.929, 17.32, &ped24, TRUE);
				CREATE_CHAR (26, PedM3, -472.366, 20.89, 12.764, &ped25, TRUE);
				CREATE_CHAR (26, PedM1, -483.657, -28.911, 7.022, &ped26, TRUE);
				CREATE_CHAR (26, PedM2, -473.936, -31.972, 5.475, &ped27, TRUE);
				CREATE_CHAR_INSIDE_CAR(car6, 1, PedM3, &ped28);
				CREATE_CHAR (26, PedM4, -461.393, 15.693, 23.907, &ped29, TRUE);
				CREATE_CHAR (26, PedM2, -455.371, 16.719, 23.907, &ped30, TRUE);
				...
				SET_CAR_PROOFS(car6, 1, 1, 1, 1, 1);//делает бесмертной
