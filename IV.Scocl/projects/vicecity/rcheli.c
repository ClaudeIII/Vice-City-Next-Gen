...
void rc_raider(void)
{
	...
	SET_PLAYER_INVINCIBLE(GetPlayerIndex(), 1);// »грок бессмертный
	...
	MARK_CAR_AS_NO_LONGER_NEEDED(&car0);//выгружаем из пам€ти
	
	//по€снени€ текстом в низу здесь
	...
	POINT_CAM_AT_COORD	( camera, -858.486, -720.329, 16.918 );
	SET_CAM_POS			( camera, -862.129, -708.642, 19.483 );
	...
	SET_PLAYER_INVINCIBLE(GetPlayerIndex(), 0);// »грок бессмертный
	...
	MARK_CAR_AS_NO_LONGER_NEEDED(&goblin);
	SetTime(1000);
