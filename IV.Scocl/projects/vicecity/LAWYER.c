...
void main(void)
{
	float PlayX, PlayY, PlayZ, heading, skip, dialog, textID, help, car1_des, car2_des, car2_des2;
	...
	uint hour, minute, weather, autosave;
	...
	while (true)
	{
		...
		if (!G_ONMISSION)
		{
			...
			if (G_LAWYER == 1)
			{
				...
				GET_DISTANCE_BETWEEN_COORDS_3D(PlayX, PlayY, PlayZ, 552.217, -293.87, 4.356, &heading);
				if ((heading < 1.5) && (!IS_CHAR_SITTING_IN_ANY_CAR(GetPlayerPed())))
				{
					...
					CHANGE_BLIP_COLOUR(Party_ico, 0);
					...
					uint MODEL_M_M_SECURITYMAN = MODEL_M_Y_DORK_02;
					uint MODEL_IG_TUNA = MODEL_IG_ANNA;
					uint MODEL_FXT = MODEL_BOBBER;
					uint CarM2 = MODEL_BANSHEE;
					uint CarM3 = MODEL_SUPERGT;
					uint CarM4 = MODEL_INFERNUS;
					uint CarM5 = MODEL_STRETCH;

					REQUEST_MODEL(MODEL_FXT);
					while (!HAS_MODEL_LOADED(MODEL_FXT)) WAIT(0);
					...
					REQUEST_MODEL(MODEL_M_M_SECURITYMAN);
					while (!HAS_MODEL_LOADED(MODEL_M_M_SECURITYMAN)); WAIT(0);

					REQUEST_MODEL(MODEL_IG_TUNA);
					while (!HAS_MODEL_LOADED(MODEL_IG_TUNA)); WAIT(0);

					CREATE_CAR(MODEL_FXT, 286.576, -788.199, 5.801, &car1, TRUE);
					CREATE_CHAR_INSIDE_CAR(car1, 1, MODEL_M_M_SECURITYMAN, &ped1);
					...
					CREATE_CHAR(26, MODEL_IG_TUNA, 281.145, -791.79, 5.445, &ped2, TRUE);
					...
					while (TRUE)
					{
						...
						GET_CHAR_COORDINATES(GetPlayerPed(), &PlayX, &PlayY, &PlayZ);
						GET_DISTANCE_BETWEEN_COORDS_3D(PlayX, PlayY, PlayZ, 527.259, -601.747, 4.341, &heading);
					...
					if (skip == 0)
					{
						while (TRUE)
						{
							...
							{
								load_all = 1;
								MARK_MODEL_AS_NO_LONGER_NEEDED(MODEL_M_M_SECURITYMAN);
								MARK_MODEL_AS_NO_LONGER_NEEDED(MODEL_FXT);
							...
							GET_CHAR_COORDINATES(GetPlayerPed(), &PlayX, &PlayY, &PlayZ);
							GET_DISTANCE_BETWEEN_COORDS_3D(PlayX, PlayY, PlayZ, 192.169, -827.01, 2.043, &heading);
							if (heading < 1.3)
							{
								REMOVE_BLIP(Party_ico);
								SET_PLAYER_CONTROL_ADVANCED(GetPlayerIndex(), 0, 0, 0);
					...
					if (skip == 0)
					{
						while (TRUE)
						{
							...
							GET_CHAR_COORDINATES(GetPlayerPed(), &PlayX, &PlayY, &PlayZ);
							GET_CHAR_COORDINATES(ped2, &PedX, &PedY, &PedZ);
							GET_DISTANCE_BETWEEN_COORDS_3D(PlayX, PlayY, PlayZ, 534.054, -946.33, 4.189, &heading);
							GET_DISTANCE_BETWEEN_COORDS_3D(PedX, PedY, PedZ, 534.054, -946.33, 4.189, &PedR);
							if ((heading < 1.5) && (PedR < 1.5))
							{
					...
					MARK_MODEL_AS_NO_LONGER_NEEDED(MODEL_IG_TUNA);
			...
			else if (G_LAWYER == 2)
			{
				...
				if ((heading < 1.5) && (!IS_CHAR_SITTING_IN_ANY_CAR(GetPlayerPed())))
				{
					...
					uint MODEL_M_M_SECURITYMAN = MODEL_M_M_KOREACOOK_01;// Повор - босс 
					uint MODEL_IG_TUNA = MODEL_M_M_SHORTORDER;// повор m_m_koreacook_01 
					uint MODEL_IG_VINNY_SPAZ = MODEL_IG_LILJACOB;// Ланс
					uint MODEL_FXT = MODEL_INFERNUS;
					...
					REQUEST_MODEL(MODEL_M_M_SECURITYMAN);// Повор - босс
					while (!HAS_MODEL_LOADED(MODEL_M_M_SECURITYMAN));
					REQUEST_MODEL(MODEL_IG_TUNA);// повор
					while (!HAS_MODEL_LOADED(MODEL_IG_TUNA));
					REQUEST_MODEL(MODEL_IG_VINNY_SPAZ);// Ланс
					while (!HAS_MODEL_LOADED(MODEL_IG_VINNY_SPAZ));

					CREATE_CHAR(26, MODEL_M_M_SECURITYMAN, 281.145, -791.79, 5.445, &ped1, TRUE);
					CREATE_CHAR(26, MODEL_IG_TUNA, 281.145, -791.79, 5.445, &ped2, TRUE);
					CREATE_CHAR(26, MODEL_IG_TUNA, 281.145, -791.79, 5.445, &ped3, TRUE);
					CREATE_CHAR(26, MODEL_IG_TUNA, 281.145, -791.79, 5.445, &ped4, TRUE);
					CREATE_CHAR(26, MODEL_IG_VINNY_SPAZ, 281.145, -791.79, 5.445, &ped5, TRUE);

					REQUEST_MODEL(MODEL_FXT);
					while (!HAS_MODEL_LOADED(MODEL_FXT)) WAIT(0);
					CREATE_CAR(MODEL_FXT, 196.987, -842.386, 2.732, &car1, TRUE);
					...
					if (skip == 0)
					{
						...
						while (TRUE)
						{
							...
							WAIT(0);
							GET_CHAR_COORDINATES(GetPlayerPed(), &PlayX, &PlayY, &PlayZ);
							GET_DISTANCE_BETWEEN_COORDS_3D(PlayX, PlayY, PlayZ, 869.546, -37.984, 5.566, &heading);
							if (heading < 15.3)
							{
								REMOVE_BLIP(Party_ico);
								SET_PLAYER_CONTROL_ADVANCED(GetPlayerIndex(), 0, 0, 0);
					...
					if (skip == 0)
					{
						while (TRUE)
						{
							...
							if (HAS_PICKUP_BEEN_COLLECTED(mob_phone))
							{
								...
								SET_CHAR_COORDINATES(ped2, 871.851, -33.606, 4.918);
								SET_CHAR_HEADING(ped2, 154.994);

								SET_CHAR_COORDINATES(ped3, 870.694, -32.833, 4.918);
								SET_CHAR_HEADING(ped3, 154.994);

								SET_CHAR_COORDINATES(ped4, 869.312, -32.151, 4.918);
								...
								CHANGE_BLIP_COLOUR(lance_car, 19);
					...
					if (skip == 0)
					{
						while (TRUE)
						{
							// сесть в иашину
							WAIT(0);
							GET_CHAR_COORDINATES(ped5, &PedX, &PedY, &PedZ);//вписываем координаты в переменную
							if (in_cord == 0)
							{
							...
							{
								GET_CHAR_COORDINATES(GetPlayerPed(), &PlayX, &PlayY, &PlayZ);
								GET_DISTANCE_BETWEEN_COORDS_3D(PlayX, PlayY, PlayZ, 868.855, -116.292, 5.08047, &heading);
								if (heading < 1.5)
								{
									FREEZE_CAR_POSITION(car1, 0);
									LOCK_CAR_DOORS(car1, 1); //разблокируем дверимобиля для
									locked = 0;
								}
							}
					...
					if (skip == 0)
					{
						while (TRUE)
						{
							...
							if (IS_CHAR_IN_CAR(GetPlayerPed(), car1))
							{
								...
								GET_CHAR_COORDINATES(GetPlayerPed(), &PlayX, &PlayY, &PlayZ);
								GET_DISTANCE_BETWEEN_COORDS_3D(PlayX, PlayY, PlayZ, 379.082, -948.858, 4.217, &heading);
								if ((heading < 2.1) && (IS_CHAR_SITTING_IN_ANY_CAR(GetPlayerPed())))
								{
									ABORT_SCRIPTED_CONVERSATION(0);
							...
							else if (!IS_CHAR_SITTING_IN_ANY_CAR(ped5))//если Ланс вышел из то убиваем его
							{
								skip = 1;
								PRINT_STRING_IN_STRING("string", "COK2_26", 5500, 1);//~r~You killed Lance!
								EXPLODE_CHAR_HEAD(ped5);
					...
					MARK_MODEL_AS_NO_LONGER_NEEDED(MODEL_M_M_SECURITYMAN);
					MARK_MODEL_AS_NO_LONGER_NEEDED(MODEL_IG_TUNA);
					MARK_MODEL_AS_NO_LONGER_NEEDED(MODEL_IG_VINNY_SPAZ);
					MARK_MODEL_AS_NO_LONGER_NEEDED(MODEL_FXT);
			...
			else if (G_LAWYER == 3)
			{
				...
				if ((heading < 1.5) && (!IS_CHAR_SITTING_IN_ANY_CAR(GetPlayerPed())))
				{
					...
					uint MODEL_M_M_SECURITYMAN = MODEL_M_M_GTRI_HI_01;
					uint MODEL_IG_TUNA = MODEL_M_M_BUSINESS_03;
					uint MODEL_IG_VINNY_SPAZ = MODEL_F_M_PJERSEY_01;
					uint PedM4 = MODEL_M_M_SWEEPER;
					uint PedM5 = MODEL_M_M_GRU2_HI_02;
					uint PedM6 = MODEL_IG_TUNA;

					uint MODEL_FXT = MODEL_SENTINEL;
					...
					REQUEST_MODEL(MODEL_IG_VINNY_SPAZ);
					while (!HAS_MODEL_LOADED(MODEL_IG_VINNY_SPAZ));
					...
					CREATE_CAR(MODEL_FXT, 968.689, 749.632, 9.099, &car1, TRUE);
					...
					CREATE_CHAR(26, MODEL_M_M_SECURITYMAN, 290.473, -785.685, 5.19, &ped1, TRUE);
					CREATE_CHAR(26, MODEL_IG_TUNA, 623.574, -493.928, 5.386, &ped2, TRUE);
					CREATE_CHAR(26, MODEL_IG_VINNY_SPAZ, 623.574, -493.021, 5.386, &ped3, TRUE);
					...
					CHANGE_BLIP_COLOUR(jury_car1, 19);
					...
					CHANGE_BLIP_COLOUR(jury_car2, 19);
					...
					while (TRUE)
					{
						...
						if (((ped2_HP < 990) && (car2_des == 0)) || ((ped3_HP < 990) && (car2_des == 0)))
						{
							if (IS_CAR_IN_AREA_3D(car2, 615.574, -502.26, 3.946, 621.074, -499.26, 6.946, 0))
							{
								...
								PANIC_SCREAM(ped3);
								TASK_ENTER_CAR_AS_DRIVER(ped2, car2, -1);// пед садится в
								...
								while (true)
								{
									...
									{
										WARP_CHAR_INTO_CAR(ped2, car2);// телепартируем за руль
										break;
									}
								}
								// очещаем зону
								...
								while (true)
								{
									...
									else if (((car2_HP < 5) && (car2_des2 == 4)) || (!IS_CHAR_SITTING_IN_ANY_CAR(ped2)))
									{
										if (IS_CHAR_SITTING_IN_ANY_CAR(ped2))
										{
											TASK_LEAVE_CAR(ped2, car2);// пед выходит из
										}
						...
						if ((car1_des == 1) && (car2_des == 1))// еслиа 1 и 2 разбиты
						{
							skip = 2;
							break;
						}
			...
			else if (G_LAWYER == 4)
			{
				...
				if ((heading < 1.5) && (!IS_CHAR_SITTING_IN_ANY_CAR(GetPlayerPed())))
				{
					...
					REQUEST_MODEL(MODEL_IG_VINNY_SPAZ);
					while (!HAS_MODEL_LOADED(MODEL_IG_VINNY_SPAZ)); WAIT(0);

					REQUEST_MODEL(barrel4);
					while (!HAS_MODEL_LOADED(barrel4)) WAIT(0);

					REQUEST_MODEL(vc_electricgate);
					while (!HAS_MODEL_LOADED(vc_electricgate)) WAIT(0);

					CREATE_OBJECT_NO_OFFSET(vc_electricgate, 731.196, 219.32, 6.078, &vorota, TRUE);
					SET_OBJECT_DYNAMIC(vorota, 0);
					SET_OBJECT_INVINCIBLE(vorota, 1);
					FREEZE_OBJECT_POSITION(vorota, 1);

					CREATE_OBJECT_NO_OFFSET(barrel4, 716.957, 222.311, 6.386, &barrel_1, TRUE);
					CREATE_OBJECT_NO_OFFSET(barrel4, 747.284, 231.978, 6.386, &barrel_2, TRUE);
					CREATE_OBJECT_NO_OFFSET(barrel4, 744.208, 221.089, 6.386, &barrel_3, TRUE);

					CREATE_CAR(MODEL_FXT, 744.774, 223.702, 6.354, &car1, TRUE);
					SET_CAR_HEADING(car1, 89.421);
					CREATE_CAR(MODEL_FXT, 744.774, 229.793, 6.354, &car2, TRUE);
					SET_CAR_HEADING(car2, 89.421);
					CREATE_CAR(MODEL_FXT, 719.85, 230.833, 6.354, &car3, TRUE);
					SET_CAR_HEADING(car3, -90.579);

					CREATE_CHAR(26, MODEL_M_M_SECURITYMAN, 737.717, 234.072, 6.354, &ped1, TRUE);
					CREATE_CHAR(26, MODEL_M_M_SECURITYMAN, 731.529, 221.274, 6.354, &ped2, TRUE);
					CREATE_CHAR(26, MODEL_M_M_SECURITYMAN, 735.911, 221.376, 6.354, &ped3, TRUE);

					CREATE_CHAR(26, MODEL_IG_TUNA, 726.113, 216.401, 6.354, &ped4, TRUE);
					CREATE_CHAR(26, MODEL_IG_TUNA, 737.019, 211.426, 6.354, &ped5, TRUE);
					CREATE_CHAR(26, MODEL_IG_TUNA, 740.795, 216.692, 6.354, &ped6, TRUE);
					CREATE_CHAR(26, MODEL_IG_TUNA, 731.361, 214.974, 6.354, &ped7, TRUE);
					CREATE_CHAR(26, MODEL_IG_TUNA, 726.873, 210.934, 6.354, &ped8, TRUE);
					CREATE_CHAR(26, MODEL_IG_TUNA, 735.413, 214.706, 6.354, &ped9, TRUE);
					CREATE_CHAR(26, MODEL_IG_TUNA, 741.986, 213.132, 6.354, &ped10, TRUE);
					CREATE_CHAR(26, MODEL_IG_TUNA, 739.796, 209.501, 6.354, &ped11, TRUE);
					CREATE_CHAR(26, MODEL_IG_TUNA, 730.041, 212.085, 6.354, &ped12, TRUE);
					CREATE_CHAR(26, MODEL_IG_TUNA, 736.509, 217.536, 6.354, &ped13, TRUE);
					CREATE_CHAR(26, MODEL_IG_TUNA, 731.682, 209.003, 6.354, &ped14, TRUE);

					CREATE_CHAR(26, MODEL_IG_VINNY_SPAZ, 723.234, 212.966, 6.354, &ped15, TRUE);
					CREATE_CHAR(26, MODEL_IG_VINNY_SPAZ, 728.926, 208.093, 6.354, &ped16, TRUE);
					CREATE_CHAR(26, MODEL_IG_VINNY_SPAZ, 726.826, 213.711, 6.354, &ped17, TRUE);
					CREATE_CHAR(26, MODEL_IG_VINNY_SPAZ, 729.155, 216.309, 6.354, &ped18, TRUE);
					CREATE_CHAR(26, MODEL_IG_VINNY_SPAZ, 732.908, 217.047, 6.354, &ped19, TRUE);
					CREATE_CHAR(26, MODEL_IG_VINNY_SPAZ, 733.013, 212.894, 6.354, &ped20, TRUE);
					CREATE_CHAR(26, MODEL_IG_VINNY_SPAZ, 734.284, 210.596, 6.354, &ped21, TRUE);
					CREATE_CHAR(26, MODEL_IG_VINNY_SPAZ, 736.751, 207.754, 6.354, &ped22, TRUE);
					CREATE_CHAR(26, MODEL_IG_VINNY_SPAZ, 738.673, 215.755, 6.354, &ped23, TRUE);
					CREATE_CHAR(26, MODEL_IG_VINNY_SPAZ, 741.959, 210.859, 6.354, &ped24, TRUE);
					...
					while (TRUE)
					{
						...
						GET_CHAR_COORDINATES(GetPlayerPed(), &PlayX, &PlayY, &PlayZ);
						GET_DISTANCE_BETWEEN_COORDS_3D(PlayX, PlayY, PlayZ, 731.166, 210.147, 6.354, &heading);
						if ((heading < 40.0) && (sound == 0))
						{
							SETTIMERB(0);
							sound = 1;
						}
						else if ((heading > 40.0) && (sound == 1))
						{
							ABORT_SCRIPTED_CONVERSATION(0);
							sound = 0;
						}
					...
					MARK_MODEL_AS_NO_LONGER_NEEDED(vc_electricgate);
