...
void launcher(void)
{
	while (TRUE)
	{
		...
		if ((G_ONMISSION == 0) && (G_CORTEZ >= 8) && (G_MAIN_CALL == 6) && (G_COUNT == 3) && (G_COKERUN == 2) && (G_BUSINES_SA >= 2) && (G_ICECREAM == 2) && (G_CABS == 4) && (G_PRON >= 5) && (G_CLUB >= 6)) // если Вся нужная недвижимость пройдина
		{
			SetTime(8000);
			if ((G_ONMISSION == 0) && (!IS_CHAR_IN_WATER(GetPlayerPed())))
			{
